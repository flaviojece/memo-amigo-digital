-- ============================================================
-- Onda 1: adesão ao tratamento + fotos de medicamento
-- ============================================================
...
-- Bucket privado para as fotos (o idoso reconhece a caixa, não o nome)
insert into storage.buckets (id, name, public)
values ('medication-photos', 'medication-photos', false)
on conflict (id) do nothing;

-- Caminho dos arquivos: <user_id>/<medication_id>.<ext>
-- Assim a primeira pasta identifica o dono e o RLS fica simples.
...
-- 2. Índice para as consultas de adesão ----------------------
create index if not exists idx_medication_logs_user_scheduled
  on public.medication_logs (user_id, scheduled_time desc);

-- 3. Resumo de adesão por dia --------------------------------
-- Retorna uma linha por dia do período, com quantas doses estavam
-- previstas e quantas foram confirmadas.
-- SECURITY DEFINER porque um anjo precisa ler os dados do paciente.
...
as $$
begin
  -- Só o próprio paciente ou um anjo ativo dele pode consultar
  if auth.uid() is null
  ...
  -- Quantas doses estavam previstas em cada dia, a partir dos horários
  -- cadastrados em cada medicamento ativo no período
  previsto as (
...
-- 4. Adesão detalhada por medicamento (para o relatório médico)
