...
const Login = () => {
  ...
  const handleSignup = async (data: SignupFormData) => {
    const {
      error
    } = await signUp(data.email, data.password, data.fullName, data.userType);
    if (!error) {
      // Check if there's a pending invitation token
      const pendingToken = sessionStorage.getItem('pendingInvitationToken');
      if (pendingToken) {
  ...
  return <div className="min-h-screen flex items-center justify-center p-4 bg-gradient-retro medical-pattern">
      <Card className="w-full max-w-md p-8 bg-radio-wood border-4 border-radio-metal shadow-floating animate-fade-in-up">
        ...
        <div className="flex gap-2 mb-6">
          <Button type="button" variant={!isSignUp ? "default" : "outline"} className="flex-1" onClick={() => setIsSignUp(false)}>
            Entrar
          </Button>
          <Button type="button" variant={isSignUp ? "default" : "outline"} className="flex-1" onClick={() => setIsSignUp(true)}>
            Cadastrar
          </Button>
        </div>

        {/* Formulário de Login */}
        {!isSignUp ? <form onSubmit={loginForm.handleSubmit(handleLogin)} className="space-y-4">
            <div>
              ...
              <div className="relative mt-2 input-glow rounded-lg">
                <Input id="email" type="email" className="h-14 text-lg bg-white" placeholder="seu@email.com" {...loginForm.register("email")} />
              </div>
                {loginForm.formState.errors.email && <p className="text-red-600 bg-white/90 px-2 py-1 rounded text-sm mt-1 font-semibold">
            ...
            <Button type="submit" className="w-full h-14 text-lg transition-all hover:scale-[1.02] hover:shadow-lg" disabled={loginForm.formState.isSubmitting}>
      ...
      <form onSubmit={signupForm.handleSubmit(handleSignup)} className="space-y-4">
            <div>
              ...
              <div className="relative mt-2 input-glow rounded-lg">
                <Input id="fullName" type="text" className="h-14 text-lg bg-white" placeholder="Seu nome" {...signupForm.register("fullName")} />
              </div>
                {signupForm.formState.errors.fullName && <p className="text-red-600 bg-white/90 px-2 py-1 rounded text-sm mt-1 font-semibold">
            ...
            <div>
              ...
              <div className="relative mt-2">
                <Select onValueChange={value => signupForm.setValue('userType', value as 'patient' | 'angel')} defaultValue="patient">
              ...
              {signupForm.formState.errors.userType && <p className="text-red-600 bg-white/90 px-2 py-1 rounded text-sm mt-1 font-semibold">
            ...
            <div>
              ...
              <div className="relative mt-2 input-glow rounded-lg">
                <Input id="signup-email" type="email" className="h-14 text-lg bg-white" placeholder="seu@email.com" {...signupForm.register("email")} />
              </div>
                {signupForm.formState.errors.email && <p className="text-red-600 bg-white/90 px-2 py-1 rounded text-sm mt-1 font-semibold">
            ...
            <div>
              ...
              <PasswordStrengthMeter password={passwordValue} />
                ...
                {signupForm.formState.errors.password && <p className="text-red-600 bg-white/90 px-2 py-1 rounded text-sm mt-1 font-semibold">
            ...
            <Button type="submit" className="w-full h-14 text-lg transition-all hover:scale-[1.02] hover:shadow-lg" disabled={signupForm.formState.isSubmitting}>
